...
const pkceStore = new Map<string, { redirectUri: string; clientId: string }>();
...
export function createHttpApp(opts: HttpAppOptions): express.Express {
  ...
  app.get('/authorize', (req, res) => {
    ...
    pkceStore.set(state, { redirectUri: redirect_uri, clientId: client_id ?? 'unknown' });
  ...
  app.get('/callback', (req, res) => {
    ...
    authCodeStore.set(code, { sessionToken: session_token, expiresAt: Date.now() + 60_000 });
  ...
  app.post('/token', (req, res) => {
    ...
    tokenStore.set(accessToken, { sessionToken: entry.sessionToken, expiresAt: Date.now() + 86_400_000 });
  ...
  app.all('/mcp', requireAuth, async (req, res) => {
    ...
    const sessionToken = (req as unknown as Record<string, unknown>).sessionToken as string;
    const apiClient = new GetMonitorClient({ baseUrl: opts.apiUrl, token: sessionToken });
    const mcpServer = createServer(apiClient);
    const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: () => generateToken(16) });
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  app.listen(PORT, () => console.log(`[GetMonitor MCP] HTTP server on :${PORT}`));
}
